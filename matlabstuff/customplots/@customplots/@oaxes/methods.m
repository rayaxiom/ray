...
elseif length(hTick) < nTicks
    ...
    newLabels = text(newLabelPos,...
        ...
        'Interpreter',OA.TickLabelInterpreter,...
        'Parent',tickParent,...
        'Tag','TickLabel',...
        'Visible','off',...
...
if isHidden
    ...
    axColor = get(OA.hAx,'color');
    figColor = get(get(OA.hAx,'parent'),'color');

    if strcmp(hideMode,'visibility') && (strcmp(axColor,'none') || ...
            all(axColor == figColor))
        % Set parent axes 'Visible' property to 'off'
        set(OA.hAx,'Visible','off')

        % Keep the title visible
        set(get(OA.hAx,'Title'),'Visible','on')

        % Restore the axes colors if necessary
        if ~isempty(OA.parentAxesColors)
            set(OA.hAx,{'XColor','YColor','ZColor'},OA.parentAxesColors)
            OA.parentAxesColors = [];
        end          
    else
        % Save axes properties to restore later
        OA.parentAxesColors = get(OA.hAx,{'XColor','YColor','ZColor'});
        OA.parentAxesTickDir = get(OA.hAx,'TickDir');
        OA.parentAxesTickDirMode = get(OA.hAx,'TickDirMode');
        
        % Now set the axes colors to the parent figure color
        set(OA.hAx,'XColor',figColor,'YColor',figColor,'ZColor',figColor);

        % Set axes TickDir to 'out'
        set(OA.hAx,'TickDir','out')
        
        % Restore parent axes' 'Visible' property
        set(OA.hAx,'Visible','on')        
    end

else
    
    % Restore the parent axes colors and tick direction if necessary
    if ~isempty(OA.parentAxesColors)
        set(OA.hAx,{'XColor','YColor','ZColor'},OA.parentAxesColors)
        OA.parentAxesColors = [];
        set(OA.hAx,'TickDir',OA.parentAxesTickDir)
        set(OA.hAx,'TickDirMode',OA.parentAxesTickDirMode)
    end
